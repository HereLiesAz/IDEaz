...
Effective and performant communication between the isolated processes is crucial.
* **Host-to-BuildService:** The **Android Interface Definition Language (AIDL)** provides a robust framework for this purpose. AIDL allows for the definition of a programmatic interface that both the client (e.Sort App) and the service (Build Service) agree upon, enabling strongly-typed, thread-safe, and high-performance method calls across process boundaries using the underlying Binder IPC mechanism.
* **Host-to-InspectionService:** Because an `AccessibilityService`'s `onBind` method is `final` and reserved by the system, AIDL cannot be used. Communication will be handled via **system-wide Broadcast Intents**. The Host App sends commands (e.g., "show prompt") as broadcasts, and the Inspection Service sends results (e.g., "prompt submitted") as broadcasts, with both components registering receivers for the other's actions.
...
The AccessibilityService API includes the ability to draw system-level overlays that appear on top of all other applications.2 When the user activates "Selection Mode" in the IDEaz, the service will perform the following actions:
...
4.  **Property Retrieval:** Once the target AccessibilityNodeInfo is identified, the service will retrieve its properties, including its on-screen bounds and, most importantly, its resource ID name (e.g., "com.example.myapp:id/login\_button").
5.  **IPC Communication:** The service signals the IDEaz Host App via `sendBroadcast()`, passing along the crucial resource ID of the selected node.
6.  **Contextual UI Trigger:** This is a key step. The Host App receives the resource ID broadcast, performs its source-map lookup, and then **commands the UIInspectionService via a new Broadcast Intent** to render a floating UI element. This UI, managed by the service, consists of a prompt for user input and a log box for AI chat feedback, positioned near the selected element.
...
## **Section 4: AI-Powered Code Generation via AI Abstraction**

The core innovation of IDEaz is its ability to translate natural language instructions into concrete code modifications. This is powered by an **abstracted AI layer**, allowing the user to choose their preferred backend (e.g., the Jules API, Google's Gemini API) for different tasks.

### **4.1 Project Management: The On-Device Git Repository**

A fundamental prerequisite for AI-driven code modification (specifically with Jules) is that the source code must be managed within a version control system. Every project created within IDEaz will be managed as a local Git repository using the embedded JGit library.

### **4.2 AI Abstraction and Routing**

The IDE is not a "Jules" app; it is an AI-agnostic platform.
* **User Configuration:** The `SettingsScreen` allows the user to provide API keys for both Jules and Google AI Studio (Gemini).
* **Task Assignment:** The user can assign a specific AI model (e.g., "Jules", "Gemini Flash") to each of the core AI tasks: "Project Initialization," "Contextless Chat," and "Overlay Chat."
* **Request Routing:** The `MainViewModel` acts as a router. Before sending any AI prompt, it:
    1.  Checks the `SettingsViewModel` for the model assigned to that specific task (e.g., `KEY_AI_ASSIGNMENT_OVERLAY`).
    2.  Verifies that the required API key for that model is present.
    3.  Dispatches the request to the corresponding API client (e.g., `ApiClient.julesApiService` or a future `GeminiApiService`).
...
2.  **Step 2: Create Session (POST /v1alpha/sessions):** The Host App initiates the process by sending an HTTP POST request to the **user-selected AI backend** for the "Overlay Chat" task.
3.  **Step 3: Stream Logs to Overlay:** The Host App polls for results. All AI-generated messages are **streamed via Broadcast Intent to the UIInspectionService**, which displays them in the floating log box.
...
6.  **Step 6: Trigger Rebuild:** The Host App sends an IPC message to the On-Device Build Service. All **build and compile logs** are streamed *only* to the Host App's main build console (the bottom sheet).
7.  **Step 7: Task Finished:** Upon build success or failure, the Host App commands the `UIInspectionService` via Broadcast to close the floating overlay UI.
...
2.  **Step 2:** The Host App checks the "Contextless Chat" assignment and creates a Jules/Gemini session.
...
The entire Host App is a **transparent, overlay activity**. This allows the user's application to be visible in the background at all times.

### **5.1 The Two-Mode Interaction Model**
The app is controlled by the state of the bottom sheet, which is also synchronized with a toggle button in the NavRail ("Interact" / "Select").

* **Interaction Mode (`AlmostHidden` sheet state):** In this mode, the Host App is invisible (or nearly invisible) and the `UIInspectionService` is stopped. The user can fully interact with their own application running underneath. Tapping the "Select" button or dragging the sheet up enters Selection Mode.
* **Selection Mode (`Peek` or `Halfway` sheet state):** In this mode, the Host App's UI (NavRail, bottom sheet) is visible, and the `UIInspectionService` is active, intercepting all touches. Tapping the screen selects a UI element. Tapping the "Interact" button or dragging the sheet down enters Interaction Mode.

### **5.2 The AI Interaction Flow**

The UI for the visual, AI-driven modification loop is seamless and intuitive, managed by the `UIInspectionService`.

* **Activation:** The user enters "Selection Mode" (e.g., swipes up the bottom sheet).
* **The Contextual Prompt/Log:** Upon selecting a UI element in the target app, the `MainViewModel` commands the `UIInspectionService` to render a floating UI. This UI is a single "chat" window:
    ...
    * If the AI (from *any* provider) needs clarification, the log box displays the question, and the **prompt input box reappears** below it, allowing the user to continue the conversation.
...
A swipe-up bottom card, accessible via a handle at the very bottom of the main workspace, provides access to detailed **global** build and debugging information.

* **Global Log:** This tab features a scrollable, auto-updating text view that displays the real-time, raw output from the `MainViewModel`'s `buildLog` flow. This single stream contains:
    1.  All build/compile output from the `BuildService`.
    2.  All AI chat logs from the **global, contextless chat input**.
    3.  All status messages from the `MainViewModel` (e.g., "Service Connected," "Source Map Loaded").
* **AI Debugger:** If a build fails, a prominent "Debug with AI" button will appear. Tapping this button will automatically copy the complete build error log, package it with a prompt like "Analyze the following Android build error, explain the root cause, and suggest a code fix," and send it to the AI assigned to the **"Contextless Chat"** task.
...
* **Implementation:** The Host App will have a **transparent background** to allow the user's app to be seen. The bottom sheet and NavRail will use standard Material 3 colors. The app will fully support both light and dark themes.36 The selection of color palettes and typography, especially within the code editor, will be carefully considered to maximize readability and reduce eye strain during long coding sessions.
...
### **Phase 6: Advanced UI/UX and AI Abstraction**

* \[ ] **6.1: Implement Dual-Log System:** Refactor the `MainViewModel` to pipe all build/status/global-AI logs into a single `buildLog` flow, while creating a separate mechanism (Broadcasts) to send contextual AI logs to the overlay.
* \[ ] **6.2: Implement Contextual AI Overlay UI:** Enhance `UIInspectionService` to render a floating UI (using `WindowManager`) for the contextual AI chat.
* \[ ] **6.3: Implement Interact/Select Mode:** Tie the `UIInspectionService` activation to the bottom sheet state.
* \[ ] **6.4: Implement AI Abstraction Layer:** Implement the `SettingsViewModel` logic for storing AI assignments and API keys, and update `MainViewModel` to route requests based on those settings.
