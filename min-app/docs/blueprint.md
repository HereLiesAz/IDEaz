# Architectural Blueprint for IDEaz (Min-App)
...
*   **Core Loop:** Run App -> Visual Select -> AI Prompt -> AI Edit -> Remote Build -> Update.
...
*   **Interaction:**
    ...
    *   **Execution:** The AI (Jules) accesses the source, implements changes, and the GitHub Workflow builds the app.

## 2. System Architecture

### 2.1 The "Remote Only" Strategy
To ensure stability and modularity, the IDE relies entirely on remote builds:
1.  **Remote Build:** GitHub CI/CD is triggered to build the app remotely.
2.  **Poll & Download:** The IDE polls GitHub for the completion of the workflow and the creation of a Release Artifact (APK).
3.  **Install:** Once the artifact is available, it is automatically downloaded and installed.
...
*   **Build:** If neither exists, trigger a workflow dispatch on GitHub.
...
*   **Synchronization:** The IDE automatically pulls changes from GitHub to display correct status, but building happens remotely.

### 2.3 The Feedback Loop
*   **Success:** App updates live.
*   **Compilation Error:**
    *   If **User Code** fails: The remote build log is analyzed. The log is sent to Jules. Jules fixes the code, commits, and the cycle repeats.
    *   If **IDE Infrastructure** fails: The log is sent to the `HereLiesAz/IDEaz` repository as a GitHub Issue with the label `jules`.
...
*   **Update Popup:** When a remote build finishes and is downloaded, a popup ("Updating, gimme a sec") appears. Text in the prompt box is auto-copied to clipboard.

### 3.2 The Console (Bottom Card)
*   **Live Logs:** A swipe-up bottom card displays the remote build log (streamed from GitHub Actions) or AI output.
...
*   **Notification:** A persistent notification displays the status of the remote build, keeping the user informed even when the app is backgrounded.

## 4. Initialization vs. Loading

### 4.1 Loading
*   **Action:** User selects a project to "Load".
*   **Process:** Clone repo (shallow), Sync changes, Fetch branches.
...
*   **Process:**
    ...
    3.  **Start Build:** Initiate the first remote build via workflow dispatch.
